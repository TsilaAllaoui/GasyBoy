...
{
    private:
        //a special register for A and F(flags)
        SpecialRegister AF;
    ...
    public:
		...
		//getters
		uint16_t get_PC();
		uint16_t getRegister(string reg);
		uint16_t get_SP();
        ...
        void LD_rr_nn(uint16_t value, string reg);

        //load the content of the nex two bytes in memory starting at an adress to the appropriate 16bits register
        void LD_rr_16(uint16_t adress, string reg);

        //load the content of a 16bit register at memory adress
        void LD_16_rr(uint16_t adress, string reg);

        //load in SP the content of HL
        void LD_SP_HL();

		//load HL with the sum of SP and the next immediate byte 
		void LD_HL_SP_n();

        //push a 16bit register content to SP
        void PUSH(string reg);

        //pop 2bytes from the adress where the SP point to and load it in a 16bit register
        void POP(string reg);


		/********8bit Arithmetic group instructions*********/
		
		//add A and an 8bit register and store the result in A
		void ADD_A_r(char reg);

		//add A and an 8bit integer and store the result in A
		void ADD_A_n(uint8_t value);

		//add A and an 8bit integer at an adress memory and store the result in A
		void ADD_A_16();

		//add the content of the operand and the carry flag with A and store it in A
		void ADC_A_r(char reg);
		void ADC_A_n(uint8_t value);
		void ADC_A_16();

		//substract the content of A and the operand and store it in A
		void SUB_r(char reg);
		void SUB_n(uint8_t value);
		void SUB_16();

		//substract the content of A and the operand along with carry flag and store it in A
		void SBC_r(char reg);
		void SBC_n(uint8_t value);
		void SBC_16();

		//AND operation on A and the operand
		void AND_r(char reg);
		void AND_n(uint8_t value);
		void AND_16();

		//OR operation on A and the operand
		void OR_r(char reg);
		void OR_n(uint8_t value);
		void OR_16();

		//XOR operation on A and the operand
		void XOR_r(char reg);
		void XOR_n(uint8_t value);
		void XOR_16();

		//compare the Accumulator and the operand
		void CP_r(char reg);
		...
		/********General Purpose Arithmetic and CPU Control group instructions*********/
		...
		//add HL and another 16bits register
		void ADD_HL_rr(string reg);

		void ADD_SP_n();

		//add HL and the carryflag with another 16bits register
		void ADC_HL_rr(string reg);

		//substract HL with the carryflag and another 16bits register
		void SBC_HL_rr(string reg);

		//increment a 16bit register
		void INC_rr(string reg);

		//decrement a 168bit register
		void DEC_rr(string reg);


		/********Rotate and Shift group instructions*********/
		...
		//shift right a 8bit register and reset bit7 
		void SRL_r(char reg);

		//shift right a value pointed by HL and reset bit7 
		void SRL_16();
		...
		//check if bit n is set in a 8bit register and set flags accordingly
		void BIT_b_r(int bit, char reg);

		//check if bit n is set in a 8bit value pointed by HL register and set flags accordingly
		...
		void JP_c_16(string condition, uint16_t adress);
		...
		void CALL_c(string condition);

		//return from subroutine
		void RET();

		//return from subroutine with condition
		void RET_c(string condition);

...
namespace gasyboy
{
    Gpu::Gpu(Mmu &mmu) : mmu(mmu)
    {
        ...
        // creating gameboy screen && renderer
    ...
    void Gpu::step(int cycles)
    {
        ...
        if (mmu._dmaRegionWritten)
        {
            // fetch all sprites if any
            for (int i = 0; i < 40; i++)
            {
                // getting the sprite's informations
                sprites[i].Y = mmu.readRam(0xFE00 + i * 4) - 16;
                sprites[i].X = mmu.readRam(0xFE01 + i * 4) - 8;
                sprites[i].tileNumber = mmu.readRam(0xFE02 + i * 4);
                sprites[i].attribute = mmu.readRam(0xFE03 + i * 4);
            }

            mmu._dmaRegionWritten = false;
        }

        // set LCD Status changes

        // managing different modes && rendering
        setLCDStatus();
    ...
    // setting LCD Status changes && managing modes
    ...
    void Gpu::showTileData()
    {
        if (mmu._currModifiedTile >= 0)
        {
            ...
            if (baseAdress == 0x8000)
                ...
                SDL_RenderCopy(VramRenderer, tilesAt9000[index - 0xFF - 1], NULL, &pos);

            drawTileMap = true;
            mmu._currModifiedTile = -1;
    ...
    void Gpu::renderWindow()
    {
        ...
        for (int k = 0; k < SCY() + 19; k++)
        {
            ...
            for (int i = currTile + 0x20 * k; i < (currTile + 0x20 * k) + 0x14; i++)
            {
                ...
                {
                    int8_t value = (int8_t)mmu.readRam(i);
    ...
    void Gpu::renderBG()
    {
        ...
        for (int i = 0; i < 32; i++)
        {
            for (int j = 0; j < 32; j++)
            {
                ...
                uint8_t value = mmu.readRam(currTile + 0x20 * i + j);
    ...
    void Gpu::renderSprites()
    {
        ...
        {
            // getting the sprite's informations
            uint8_t yPos = mmu.readRam(0xFE00 + i) - 16;
            uint8_t xPos = mmu.readRam(0xFE01 + i) - 8;
            uint8_t tileNumber = mmu.readRam(0xFE02 + i);
            uint8_t attributes = mmu.readRam(0xFE03 + i);
    ...
    void Gpu::renderCurrScanline(int line)
    {
        ...
        for (int j = SCX() / 8; j <= SCX() / 8 + 20; j++)
        {
            ...
            uint8_t value = mmu.readRam(adress);
            ...
            {
                int8_t value = (int8_t)mmu.readRam(adress);
                       ...
                       for (int i = 0; i < 8; i++)
                        ...
                        if (line >= yPos && line <= yPos + 8)
                        {
                            if (verticalFlip)
                                ...
                                SDL_RenderCopyEx(screenRenderer, tilesForScreenAt8000[sprites[i].tileNumber], &spriteSrc, &spriteDst, 0, NULL, SDL_FLIP_NONE);
                        }
    ...
    //     {
    //         uint8_t B = mmu.readRam(i);
    //         uint8_t A = mmu.readRam(i + 1);
    ...
    void Gpu::renderTile(uint16_t adress, SDL_Rect *pos, uint16_t colorAdress, bool priority, bool Xflip, bool Yflip)
    {
        ...
        std::vector<uint32_t> palette = getPalette(colorAdress);
    ...
    std::vector<uint32_t> Gpu::getPalette(uint16_t adress)
    {
        uint8_t value = mmu.readRam(adress);
        std::vector<uint32_t> palette(4, 0);
        int k = 0;

        for (int i = 0; i < 8; i += 2)
        {
            switch ((static_cast<uint8_t>(getBitValAt(value, i)) << 1) | static_cast<uint8_t>(getBitValAt(value, i + 1)))
            {
            case 0:
                palette[k] = basePalette[0];
                break;

            case 1:
                palette[k] = basePalette[1];
                break;

            case 2:
                palette[k] = basePalette[2];
                break;

            case 3:
                palette[k] = basePalette[3];
                break;
            }
    ...
    Uint32 Gpu::getPixelColor(int x, int y, int bit)
    {
        ...
        uint8_t A = mmu.readRam(j);
        uint8_t B = mmu.readRam(j + 1);
        uint8_t color = (static_cast<uint8_t>(getBitValAt(A, bit)) << 1) | static_cast<uint8_t>(getBitValAt(B, bit));
    ...
    uint8_t Gpu::SCX()
    {
        return mmu.readRam(0xFF43);
    }

    uint8_t Gpu::SCY()
    {
        return mmu.readRam(0xFF42);
    }

    uint8_t Gpu::WX()
    {
        return mmu.readRam(0xFF4B);
    }

    uint8_t Gpu::WY()
    {
        return mmu.readRam(0xFF4A);
    }

    uint8_t Gpu::LCDC()
    {
        return mmu.readRam(0xFF40);
    }

    uint8_t Gpu::LCDSTAT()
    {
        return mmu.readRam(0xFF41);
    }

    void Gpu::setSCX(uint8_t value)
    {
        mmu.writeRam(0xFF43, value);
    }

    void Gpu::setSCY(uint8_t value)
    {
        mmu.writeRam(0xFF42, value);
    }

    void Gpu::setWX(uint8_t value)
    {
        mmu.writeRam(0xFF4B, value);
    }

    void Gpu::setWY(uint8_t value)
    {
        mmu.writeRam(0xFF4A, value);
    }

    void Gpu::setLCDC(uint8_t value)
    {
        mmu.writeRam(0xFF40, value);
    }

    void Gpu::setLCDSTAT(uint8_t value)
    {
        mmu.writeRam(0xFF41, value);
    }
    ...
    {
        uint8_t req = mmu.readRam(0xFF0F);
        req |= (1 << id);
        mmu.writeRam(0xFF0F, req);
    }
